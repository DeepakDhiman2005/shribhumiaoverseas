...
const ShowProducts = () => {
    ...
    const subcategory = useMemo(() => (search.get('subcategory')), [search]);
    ...
    const capitalize = (str: string) => {
        ...
        let newStr: string = _arr.map((item) => (item.replace(item.charAt(0), item.charAt(0).toUpperCase()))).join(' ');
        return newStr;
    }

    const heading = useMemo(() => {
        let category = search.get('category') || null;
        return `${capitalize(category?.split('-').join(' ') as string)}`;
    }, [search]);
    ...
    return (
        <main className="w-full md:w-[80%]">
            <h2 className="font-semibold text-[30px] text-green-700">{heading}<span className="text-[22px] text-gray-900">{subcategory ? `  > ${capitalize(subcategory.split('-').join(" ") as string)}`: ""}</span></h2>

            <p className="text-start w-full my-3">
                {description}
            </p>
